...
def bag_of_words(text):
    ...
    bag_of_words = Counter(list_of_text)
...
def aggregate_counts(bags_of_words):
    ...
    # YOUR CODE GOES HERE
    whole_bag = list(bags_of_words)[0]
    
    for bag in list(bags_of_words)[1:]:
        whole_bag = sum((whole_bag, bag), Counter())
...
def compute_oov(bow1, bow2):
    ...
    voc1 = set(list(bow1.keys()))
    voc2 = set(list(bow2.keys()))
...
def prune_vocabulary(training_counts, target_data, min_counts):
    ...
    for word in training_counts:
        if training_counts[word] >= min_counts:
            t_data.add(word)
            pruned_bag[word] = training_counts[word] 
    
    return pruned_bag, t_data
...
def make_feature_vector(base_features,label):
    ...
    '''
    feature_vector = {(label, OFFSET): 0}
    
    for item in base_features.items():
        feature_vector[(label, item[0])] = item[1]
